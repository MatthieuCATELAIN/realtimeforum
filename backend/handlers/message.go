...
func MessageHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		...
		messages, err := FindChatMessages(Path, s, r, firstId)
		if err != nil {
	...
	case "POST":
		...
		err = NewMessage(Path, newMessage)
		if err != nil {
			http.Error(w, "500 internal server error", http.StatusInternalServerError)
			return
		}
...
func NewMessage(path string, m Message) error {
	db, err := OpenDB(path)
	if err != nil {
		return err
	}
...
func FindChatMessages(path, sender, receiver string, firstId int) ([]Message, error) {
	db, err := OpenDB(path)
	if err != nil {
		return []Message{}, errors.New("failed to open database")
	}
	defer db.Close()
...
func FindLastMessage(path, sender, receiver string) (Message, error) {
	// Opens the database
	db, err := OpenDB(path)
	if err != nil {
		return Message{}, errors.New("failed to open database")
